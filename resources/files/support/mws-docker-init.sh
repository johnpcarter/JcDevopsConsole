...
if [ $USER == 'sagadmin' ]
then
	/opt/softwareag/profiles/MWS_default/bin/startup.sh
else
	su wm -c '/opt/softwareag/profiles/MWS_default/bin/startup.sh'
fi
...
ALERT_LOG=/opt/softwareag/profiles/MWS_default/logs/wrapper.log
tail -f $ALERT_LOG &
childPID=$!
wait $childPID
